...
<div id="main">
  ...
  <center> 
    ...
    <center>   <asp:Label ID="Label1" runat="server" Text="VIDEO CONTROL" Font-Underline="true" Font-Size="X-Large" ForeColor="#CCCCCC"></asp:Label></center> 
        <br>
        <br>
        <asp:GridView ID="GridView1" runat="server" AllowPaging="True" AutoGenerateColumns="False" DataKeyNames="Id" DataSourceID="SqlDataSource1" Width="859px" Height="573px" PageSize="20" >
          <Columns>
              ...
              <asp:BoundField DataField="ContentType" HeaderStyle-HorizontalAlign="Justify" HeaderText="ContentType" SortExpression="ContentType" HeaderStyle-Height="30" HeaderStyle-Font-Bold="false" HeaderStyle-Font-Size="Larger" />
              <asp:BoundField DataField="Data" HeaderStyle-HorizontalAlign="Justify" HeaderText="Data" SortExpression="Data" HeaderStyle-Height="30" HeaderStyle-Font-Bold="false" HeaderStyle-Font-Size="Larger" />
          </Columns>
      </asp:GridView>
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:gurpreetConnectionString %>" DeleteCommand="DELETE FROM [vid_path] WHERE [Id] = @original_Id AND (([Name] = @original_Name) OR ([Name] IS NULL AND @original_Name IS NULL)) AND (([ContentType] = @original_ContentType) OR ([ContentType] IS NULL AND @original_ContentType IS NULL)) AND (([Data] = @original_Data) OR ([Data] IS NULL AND @original_Data IS NULL))" InsertCommand="INSERT INTO [vid_path] ([Name], [ContentType], [Data]) VALUES (@Name, @ContentType, @Data)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT * FROM [vid_path]" UpdateCommand="UPDATE [vid_path] SET [Name] = @Name, [ContentType] = @ContentType, [Data] = @Data WHERE [Id] = @original_Id AND (([Name] = @original_Name) OR ([Name] IS NULL AND @original_Name IS NULL)) AND (([ContentType] = @original_ContentType) OR ([ContentType] IS NULL AND @original_ContentType IS NULL)) AND (([Data] = @original_Data) OR ([Data] IS NULL AND @original_Data IS NULL))">
            <DeleteParameters>
